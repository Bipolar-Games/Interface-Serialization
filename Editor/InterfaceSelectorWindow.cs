...
namespace Bipolar.Editor
{
    ...
    public class InterfaceSelectorWindow : EditorWindow
    {
        ...
        private static Component[] GetComponentsOfInterface(System.Type interfaceType)
        {
            ...
            foreach (var gameObject in GetAllGameObjects())
            {
        ...
        private static IEnumerable<GameObject> GetAllGameObjects()
        {
            var prefabStage = PrefabStageUtility.GetCurrentPrefabStage();
            if (prefabStage)
            {
                var prefabRoot = prefabStage.prefabContentsRoot;
                var gameObjectsInPrefab = prefabRoot.GetComponentsInChildren<Transform>()
            ...
            {
                var hierarchyProperty = new Hierarchy(HierarchyType.GameObjects);
                while (hierarchyProperty.Next(null))
                {
                    var gameObject = hierarchyProperty.pptrValue as GameObject;
                    if (gameObject)
                        yield return gameObject;
                }
            }
